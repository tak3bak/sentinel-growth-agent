...
def generate_lead_pitch(payload: TargetCreateRequest, db: Session = Depends(get_db)):
    existing_lead = db.query(TargetLead).filter(TargetLead.domain == payload.domain).first()
    recon_results = GrowthPitchEngine.simulate_recon(payload.domain)
    company = payload.company_name or payload.domain
    pitch_text = GrowthPitchEngine.synthesize_pitch(company, payload.domain, recon_results)
    
    if existing_lead:
        existing_lead.risk_score = recon_results["risk_score"]
        existing_lead.vulnerabilities_summary = " | ".join(recon_results["vulnerabilities"])
...
def list_leads(db: Session = Depends(get_db)):
    leads = db.query(TargetLead.id, TargetLead.domain, TargetLead.company_name, TargetLead.risk_score, TargetLead.status).all()
